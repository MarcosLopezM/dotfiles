...
local function make_paren_snip(trig, size)
  return s(
    { trig = trig .. "p", wordTrig = false }, -- "p" for parentheses
...
local function make_bracket_snip(trig, size)
  return s(
    { trig = trig .. "b", wordTrig = false }, -- "b" for brackets
    { t(size .. "l["), i(1), t(size .. "r]") },
    { condition = in_mathzone }
  )
end
...
local function make_brace_snip(trig, size)
  return s(
    { trig = trig .. "c", wordTrig = false }, -- "c" for curly
...
local math_envs = {
  ...
  s({ trig = "=", wordTrig = false }, {
    ...
    condition = function()
      return tex.in_mathzone and (env("align") or env("align*") or env("alignat") or env("alignat*"))
    end,
  }),
  ...
  -- New line in align
  s(
    { trig = "([,.])ll", regTrig = true, snippetType = "autosnippet", wordTrig = false },
    fmta(
      [[
              <>\\
              <>
    ...
    {
      condition = function()
        return tex.in_mathzone and (env("align") or env("align*") or env("dcases"))
      end,
    }
  ),
  ...
  s(
    { trig = "([,.])nll", regTrig = true, snippetType = "autosnippet", wordTrig = false },
    fmta(
      [[
              <>\nonumber\\
              <>
  ...
  s(
    ...
    fmta("\\<>\\<><><>\\<>", {
      ...
      f(function(_, snip)
        ...
        local map = { p = ")", b = "]", c = "}" }
        return delims[snip.captures[2]] .. "r" .. map[snip.captures[3]]
      end),
    }),
    { condition = tex.in_mathzone }
  ),
  ...
  s(
    { trig = "sin", desc = "Sine function without delimiters", wordTrig = false },
    fmta([[\sin <>]], { i(1) }),
    { condition = tex.in_mathzone }
  ),

  s(
    { trig = "cos", desc = "Cosine function without delimiters", wordTrig = false },
    fmta([[\cos <>]], { i(1) }),
    { condition = tex.in_mathzone }
  ),
  ...
  s(
    {
      trig = "(cos)([b%u]gg?)([bp])",
      regTrig = true,
      desc = "Cosine with multisize delimiters",
      priority = 900,
    },
    fmta("\\<>\\<><><>\\<>", {
      f(function(_, snip)
...
\end{{itemize}}
  ...
  s({ trig = "uu", wordTrig = false, desc = "Superscript" }, { t("^{"), i(1), t("}") }, { condition = in_mathzone }),
  s({ trig = "sb", wordTrig = false, desc = "Subscript" }, { t("_{"), i(1), t("}") }, { condition = in_mathzone }),
  s(
    { trig = "ss", desc = "Superscript and subscript", wordTrig = false },
    fmta(
      [[
              _{<>}^{<>}
            ]],
  ...
  s(
    { trig = "eu", desc = "Unit vector e", wordTrig = false },
    fmta(
      [[
      \uvec{e}_{<>}
      ]],
